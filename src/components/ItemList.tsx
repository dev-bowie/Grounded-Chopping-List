...
export function ItemList() {
    ...
    const handleUpdateAmount = (name: string, newAmount: number) => {
        setItems(prevItems =>
            prevItems.map(item =>
    ...
    const handleUpdateNote = (name: string, newNote: string) => {
        setItems(prevItems =>
            prevItems.map(item =>
    ...
    return (
        <div className='w-full mx-auto px-4 py-8'>
                            ...
                            <div
                                ...
                                {items.map(item => (
                                    <SortableItem
                                        ...
                                        onUpdateAmount={(newAmount: number) => handleUpdateAmount(item.name, newAmount)}
                                        onUpdateNote={(newNote: string) => handleUpdateNote(item.name, newNote)}
                                    />
                                ))}
                            </div>
                        </SortableContext>
            ...
            {undoItem && (
                <div
                    className='fixed bottom-6 left-1/2 transform -translate-x-1/2 bg-amber-200 text-black px- rounded shadow-lg flex items-center gap-4 z-50'>
...
function SortableItem({item, onDone, onRemove, onUpdateAmount, onUpdateNote}: {
    item: Item;
    onDone: () => void;
    onRemove: () => void;
